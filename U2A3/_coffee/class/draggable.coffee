class Draggable
	...
	putInPlace: (position, alpha=1, scaleX=1, scaleY=1) ->
		...
		if dealersjs.mobile.isIOS() or dealersjs.mobile.isAndroid()
			@set {x: position.x, y: position.y, alpha: alpha, scaleX: scaleX, scaleY: scaleY}
		else
	...
	returnToPlace: (alpha=1,scaleX=1,scaleY=1, blink=false) ->
		if not blink
			if dealersjs.mobile.isIOS() or dealersjs.mobile.isAndroid()
				@set {x: @pos.x, y: @pos.y, alpha: alpha, scaleX: scaleX, scaleY:scaleY}
			else
				TweenLite.to @, 0.5, { ease: Back.easeOut, delay: 0.1, x: @pos.x, y: @pos.y, alpha: alpha, scaleX: scaleX, scaleY: scaleY }
		else
			if dealersjs.mobile.isIOS() or dealersjs.mobile.isAndroid()
				@set {x: @pos.x, y: @pos.y, alpha: alpha, scaleX: scaleX, scaleY:scaleY, onComplete: @blinkAgain}
			else

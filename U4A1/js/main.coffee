class U4A1 extends Oda
	...
	introEvaluation: ->
		...
		if @round is 0
			TweenLite.from @library['cartaspc'], 1, {alpha: 0, y: @library['cartaspc'].y - 100, delay: 3, onComplete: @playInstructions, onCompleteParams: [@]}
		else
			TweenLite.from @library['cartaspc'], 1, {alpha: 0, y: @library['cartaspc'].y - 100, delay: 3, onComplete: @initEvaluation, onCompleteParams: [@]}

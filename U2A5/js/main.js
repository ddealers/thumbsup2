(function() {
  ...
  U2A5 = (function(_super) {
    ...
    U2A5.prototype.setStage = function() {
      ...
      this.addToMain(ti);
      btnnext = new Button('btnnext', this.preload.getResult('btn'), 0, 1220, 1178);
    ...
    U2A5.prototype.setCuento = function(scene) {
      ...
      for (i = _i = 1, _ref = this.game[scene - 1].positions.length; _i <= _ref; i = _i += 1) {
        if (scene === 1) {
          if (i === 2 || i === 4) {
            sp = this.createSprite("sp" + i, ["" + ((scene - 1) * 4 + i), "" + ((scene - 1) * 4 + i) + "p1", "" + ((scene - 1) * 4 + i) + "p2", "" + ((scene - 1) * 4 + i) + "b"], null, 0, 0);
